...
public class MashTunRecipe extends VanillaRecipeWrapper {
	@Nonnull
	...
	public MashTunRecipe(@Nonnull List<ItemStack> input, @Nonnull List<ItemStack> fuel, @Nonnull ItemStack output, float experience) {
		this.input = Collections.singletonList(input);
		this.fuel = Collections.singletonList(fuel);
		this.outputs = Collections.singletonList(output);

		if (experience > 0.0) {
			experienceString = Translator.translateToLocalFormatted("Mash Tun", experience);
		} else {
