...
public class Block_Grape extends BlockBush implements IGrowable{
	...
	private void dropcorn(World world, BlockPos pos, IBlockState state){
		ItemStack out = new ItemStack(ItemHandler.WineSeeds);
		...
		if( s == 10 || s == 11 ){ 
			...
			if( chance >= .40){
				out = new ItemStack(ItemHandler.Wine, 2);
			}else if( chance >=.1){
				out = new ItemStack(ItemHandler.Wine);
			}
		}
		...
		else
		{
			...
			if(s == 9 || s == 10 || s == 11 ){ //if corn is ripe
				if(world.rand.nextFloat() >= .40){
					out = new ItemStack(ItemHandler.Wine,1);
				}else{
					out = new ItemStack(ItemHandler.Wine);
				}

			}else{
				out = new ItemStack(ItemHandler.WineSeeds);
			}
