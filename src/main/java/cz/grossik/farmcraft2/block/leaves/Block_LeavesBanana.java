...
public class Block_LeavesBanana extends BlockLeaves {

	public Block_LeavesBanana() {
	...
	{
		return Item.getItemFromBlock(BlockHandler.SaplingBanana);
	}
	...
	public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
    {
        if (!worldIn.isRemote)
        {
            ...
            {
                worldIn.setBlockState(pos, BlockHandler.leavesBananaNormal.getDefaultState());
            }
            else
            {
                if (worldIn.getLightFromNeighbors(pos.up()) >= 9)
                {
                    for (int i = 0; i < 1; ++i)
                    {
                        ...
                        if (iblockstate1.getBlock() == BlockHandler.leavesBananaNormal && worldIn.getLightFromNeighbors(blockpos.up()) >= 4 && iblockstate.getLightOpacity(worldIn, pos.up()) <= 2)
                        {
                            worldIn.setBlockState(blockpos, BlockHandler.leavesBananaSB.getDefaultState());
                        }
                    }
                }
            }
