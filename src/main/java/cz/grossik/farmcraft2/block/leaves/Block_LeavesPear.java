...
public class Block_LeavesPear extends BlockLeaves {

	public Block_LeavesPear() {
	...
	{
		return Item.getItemFromBlock(BlockHandler.SaplingPear);
	}
	...
	public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
    {
        if (!worldIn.isRemote)
        {
            ...
            {
                worldIn.setBlockState(pos, BlockHandler.leavesPearNormal.getDefaultState());
            }
            else
            {
                if (worldIn.getLightFromNeighbors(pos.up()) >= 9)
                {
                    for (int i = 0; i < 1; ++i)
                    {
                        ...
                        if (iblockstate1.getBlock() == BlockHandler.leavesPearNormal && worldIn.getLightFromNeighbors(blockpos.up()) >= 4 && iblockstate.getLightOpacity(worldIn, pos.up()) <= 2)
                        {
                            worldIn.setBlockState(blockpos, BlockHandler.leavesPearSH.getDefaultState());
                        }
                    }
                }
            }
