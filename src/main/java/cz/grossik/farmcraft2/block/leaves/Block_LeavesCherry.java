...
public class Block_LeavesCherry extends BlockLeaves {
	...
	public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
    {
        if (!worldIn.isRemote)
        {
            ...
            {
                worldIn.setBlockState(pos, BlockHandler.leavesCherryNormal.getDefaultState());
            }
            else
            {
                if (worldIn.getLightFromNeighbors(pos.up()) >= 9)
                {
                    for (int i = 0; i < 1; ++i)
                    {
                        ...
                        if (iblockstate1.getBlock() == BlockHandler.leavesCherryNormal && worldIn.getLightFromNeighbors(blockpos.up()) >= 4 && iblockstate.getLightOpacity(worldIn, pos.up()) <= 2)
                        {
                            worldIn.setBlockState(blockpos, BlockHandler.leavesCherryST.getDefaultState());
                        }
                    }
                }
            }
